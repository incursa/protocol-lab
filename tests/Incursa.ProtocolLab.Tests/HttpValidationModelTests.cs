...
public sealed class HttpValidationModelTests
{
    ...
    public void Managed_h3_proof_validation_passes_with_http3_response()
    {
        ...
        var response = new ManagedProofResponse(
            ...
            ["Loopback certificate validation bypass was used for managed HTTP/3 proof."]);
        ...
        Assert.Equal("managed-httpclient-h3-exact", result.ProtocolProof!.Method);
        Assert.Equal("managed-dotnet-httpclient", result.ProtocolProof.ProofClient);
