...
- `adapter/v1/` - Adapter Contract v1 control-plane payloads.
- `artifact/v1/` - artifact references, artifact bundles, and redaction state.
- `load-profile/` - declarative load profile documents.
- `measurement/v1/` - measurement profiles, telemetry bundles, samples,
  summaries, collector descriptors, and comparability classes.
- `package/v2/` - component package manifests.
- `public-report/v1/` - evidence reports, publication manifests, report
  indexes, and artifact indexes.
- `run-plan/v1/` - immutable run-plan selection documents.
- `suite/v1/` - declarative suite selection documents.
- `test-executor/v1/` - Test Executor Contract v1 control-plane payloads.
- `scenario.schema.json` - scenario definition documents.
